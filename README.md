...
Stop-and-interact moments ("pockets") sit inside the film for anyone who wants to go deeper — the game, the music video, the jukebox, the alien-prank clip — and a permanent, quiet fast lane (Resume · Experience · Projects · Contact) is always one click away for anyone who only needs the professional facts.
...
- **Tests:** 290 unit tests (vitest) pin the scene rigs, seams between chapters, envelopes, layout purity, and the segment map against the CSS journey length; `tsc -b` is clean.
- **Tooling in the repo:** a Playwright screenshot/byte-sweep harness (`site/scripts/qa.mjs`), a deterministic seek-stepped film recorder (`site/scripts/record-film.mjs`), and the asset pipeline (`site/scripts/prepare-assets.mjs`, `grade.mjs`) that grades and stages photographs, depth maps, and loops.
